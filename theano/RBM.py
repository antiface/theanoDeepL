...
class RBM(object):
    ...
    def __init__(self, input=None, n_visible=784, n_hidden=500, \
        W=None, hbias=None, vbias=None, visible_type='binary', numpy_rng=None,
        theano_rng=None):
        """
        ...
        :param visible_type: Can be 'binary' or 'poisson'
    ...
    def sample_v_given_h_poisson(self, h0_sample):
        ''' This function infers state of visible units given hidden units '''
        # compute the activation of the visible given the hidden sample
        pre_sigmoid_v1, v1_mean = self.propdown(h0_sample)
        # get a sample of the visible given their activation
        ...
        # int64 by default. If we want to keep our computations in floatX
        # for the GPU we need to specify to return the dtype floatX
        # http://deeplearning.net/software/theano/library/tensor/raw_random.html?highlight=poisson
        v1_sample = self.theano_rng.poisson(size=pre_sigmoid_v1.shape, lam=pre_sigmoid_v1 / T.sum(pre_sigmoid_v1), dtype=theano.config.floatX)
        return [pre_sigmoid_v1, v1_mean, v1_sample]
    ...
    def get_cost_updates(self, lr=0.1, k=1):
        ...
        [pre_sigmoid_nvs, nv_means, nv_samples,
         pre_sigmoid_nhs, nh_means, nh_samples], updates = theano.scan(self.gibbs_hvh,
                    # the None are place holders, saying that
                    # ph_sample (i.e. chain_start) is the initial state corresponding to the
                    # 6th output
